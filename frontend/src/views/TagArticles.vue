  ...
  <MainLayout>
    <div class="page">
      <header class="page-header">
        ...
        <h1 class="page-title">#{{ tagName || '加载中...' }}</h1>
      </header>
      <div class="articles-list">
        ...
        <p class="empty-state" v-if="!articles.length && !loading">该标签暂无文章</p>
...
onMounted(async () => {
  try {
    const [tdata, arts] = await Promise.all([
      getTags(),
      getArticles({ tags: route.params.id })
    ])
    const tlist = Array.isArray(tdata) ? tdata : (tdata.results || [])
    const found = tlist.find(t => t.id == route.params.id)
    tagName.value = found?.name || ''
